...
import (
	"fmt"
	"net"
	"github.com/limingxinleo/gorpc/proto"
	"github.com/limingxinleo/gorpc/handler"
	"github.com/limingxinleo/gorpc/utils"
	"reflect"
)
...
func handle(conn *net.TCPConn) {
	...
	//使用方法名字符串调用指定方法
	var function = handler.ServiceMappers[service][method]
...
func (*Rpc) RegisterHandler(name string, serviceHandler handler.ServiceInterface) {
	handler.RegisterService(name, serviceHandler)
}

func (rpc *Rpc) Run() {
	localAddress, _ := net.ResolveTCPAddr("tcp4", rpc.Address) //定义一个本机IP和端口。
